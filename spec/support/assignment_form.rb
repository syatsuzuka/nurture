class AssignmentForm
  ...
  def fill_in_review_with(params = {})
    select(params.fetch(:assignment_status, "Closed"), from: 'assignment_status')
    fill_in('assignment_review_comment', with: params.fetch(:assignment_comment, "test_review_comment"))
    find('#assignment_start_date').set(params.fetch(:assignment_start_date, "2022-01-01"))
    find('#assignment_end_date').set(params.fetch(:assignment_end_date, "2022-12-31"))
    self
  end
